...
# ⏰ HORÁRIO LIMITE DE ENTRADA (Altere se necessário)
HORA_LIMITE_ENTRADA = time(7, 30)

ultimas_entradas = {}

def carregar_alunos_supabase():
    """Busca alunos no Supabase e baixa as fotos para comparação."""
    try:
        ...
        print(f"\n[TOTAL] {len(alunos)} aluno(s) carregado(s) com sucesso!\n")
        return alunos

    except Exception as e:
        print(f"[ERRO CRITICO] Falha no Supabase: {e}")
        return []

def registrar_entrada_banco(aluno_id, status):
    """Grava a entrada no Supabase evitando duplicatas consecutivas (60s)."""
    agora = datetime.now()
    ...
    try:
        ...
        print(f"[BANCO] Entrada registrada para ID {aluno_id} ({status})")
    except Exception as e:
        print(f"[AVISO] Não foi possível salvar na tabela 'entradas': {e}")
...
def iniciar_camera():
    ...
    # Carrega o classificador de rostos oficial
                ...
                if not detector.empty():
                    ...
                    for (x, y, w, h) in rostos:
                        ...
                        for aluno in alunos:
                            ...
                            # Limiar flexível para variação de luz (0.60)
                            if min_val < 0.60:
                        ...
                        # Desenha retângulo e nomes no frame
